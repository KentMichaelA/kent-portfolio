...
import { Github, Mail, MapPin, ExternalLink, Terminal, Shield, Cpu, ChevronRight } from 'lucide-react';
import { personalInfo, experiences, projects, certifications } from './portfolioData';
...
const Section = ({ title, children, id }) => (
  <section id={id} className="py-20 px-6 max-w-6xl mx-auto">
    ...
    >
      <span className="text-slate-500">0{id === "about" ? 1 : id === "experience" ? 2 : id === "projects" ? 3 : 4}.</span> 
      {title}
    ...
    <div className="min-h-screen bg-slate-900 text-slate-300 selection:bg-cyan-500/30">
      ...
      <nav className="fixed w-full bg-slate-900/90 backdrop-blur-sm z-50 border-b border-slate-800">
        <div className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
          <div className="text-cyan-400 font-bold text-xl tracking-tighter border border-cyan-400/30 p-1 px-3 rounded">
            KMA
          </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project) => (
            <motion.div 
              whileHover={{ y: -5 }}
              key={project.id} 
              className="bg-slate-800/50 p-6 rounded hover:shadow-xl hover:shadow-cyan-500/10 transition-all border border-transparent hover:border-cyan-500/30 group"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {certifications.map((cert, index) => (
            <motion.div 
              whileHover={{ y: -5 }}
              key={index} 
              className="bg-slate-800/30 p-6 rounded-xl border border-slate-700/50 hover:border-cyan-500/30 transition-all flex flex-col items-center text-center gap-4"
            >
              {/* 1. Name at the top */}
              <h4 className="font-bold text-slate-100 text-lg h-12 flex items-center justify-center">
                {cert.name}
              </h4>

              {/* 2. Image in the middle (Certificate Preview Mode) */}
              <div className="w-full aspect-video bg-slate-900 rounded-lg overflow-hidden border border-slate-700 relative">
                <img 
                  src={cert.image} 
                  alt={cert.issuer} 
